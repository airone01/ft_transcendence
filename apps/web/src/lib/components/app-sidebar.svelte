...
<Sidebar collapsible="icon">
  <SidebarHeader class="border-b flex flex-row gap-2 select-none p-2 items-center h-11">
    ...
    <p class="font-bold font-sans group-data-[collapsible=icon]:hidden shrink overflow-clip min-w-0 max-w-full">{m.app_name()}</p>
  </SidebarHeader>
  <SidebarContent>
  ...
  <SidebarFooter class="p-0 flex flex-col *:border-t gap-0 group-data-[collapsible=icon]:border-t">
    <div class="p-4 pt-1 group-data-[collapsible=icon]:hidden hover:bg-accent/10 transition-all">
      <SidebarGroupLabel>{m.nav_item_quickplay()}</SidebarGroupLabel>
      <div class="flex flex-col gap-2">
        <Button href="/play" class="overflow-clip"><ZapIcon/>{m.nav_item_mm()}</Button>
        <Button href="/play/bot" variant="outline" class="overflow-clip group-hover:bg-accent/10 hover:bg-accent/30"><BotIcon/>{m.nav_item_playbot()}</Button>
      </div>
    </div>
    <UserItem {logoutForm} />
  </SidebarFooter>
